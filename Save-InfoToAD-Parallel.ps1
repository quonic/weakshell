...
function Save-InfoToAD
{
    ...
    Process
    {
        ...
        if($Credential){
            Set-ADDescription -Computer $Computer -LogFile $LogFile -Credential $Credential
        }else{
            Set-ADDescription -Computer $Computer -LogFile $LogFile
        }
    }
    End
    {
...
Param
    ...
    if ( -not $LogFile) {$LogFile = "C:\Scripts\Logs\$env:computername.log"}
